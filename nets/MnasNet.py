...
class MnasNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = torch.flatten(x, 1)
        # x = self.classifier(x)
        feat1 = self.interverted_residual_setting[  :1 ](x)
        feat2 = self.interverted_residual_setting[1 :2 ](feat1)
        feat3 = self.interverted_residual_setting[2 :3](feat2)
        feat4 = self.interverted_residual_setting[3:4](feat3)
        feat5 = self.interverted_residual_setting[4:-1](feat4)
        return [feat1, feat2, feat3, feat4, feat5]

    def MnasNet(pretrained=False, **kwargs):
        model = MnasNet(InvertedResidual, [3, 4, 6, 3], **kwargs)