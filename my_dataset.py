...
class DUTSDataset(data.Dataset):
    def __init__(self, root: str, train: bool = True, transforms=None):
        ...
        if train:
            self.image_root = os.path.join(root, "dataset/DUTS-TR", "DUTS-TR-Image")
            self.mask_root = os.path.join(root, "dataset/DUTS-TR", "DUTS-TR-Mask")
        else:
            self.image_root = os.path.join(root, "dataset/DUTS-TE", "DUTS-TE-Image")
            self.mask_root = os.path.join(root, "dataset/DUTS-TE", "DUTS-TE-Mask")
    ...
    def __getitem__(self, idx):
        ...
        assert image is not None, f"failed to read images: {image_path}"
